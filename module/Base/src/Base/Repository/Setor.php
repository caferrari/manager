...
class Setor extends AbstractRepository
{
    ...
    {
        $orgao = $this->getEntityManager()->getReference('Base\Entity\Orgao', $orgao_id);
        // return $this->findBy(array('orgao' => $orgao, 'parent' => null), array('nome' => 'ASC'));

        $dql = 'SELECT s, p FROM Base\\Entity\\Setor s LEFT JOIN s.parent p WHERE s.orgao=?1 ORDER BY s.nome, p.nome';
